  ...
  #+begin_src emacs-lisp
    (use-package mu4e
      ...
      (add-to-list 'mu4e-bookmarks
		   (make-mu4e-bookmark
		    :name "Linux-USB"
		    :query "list:linux-usb.vger.kernel.org AND flag:unread AND NOT flag:trashed"
		    :key ?U))

      (add-to-list 'mu4e-bookmarks
		   (make-mu4e-bookmark
		    :name "Linux ARM MSM"
		    :query "list:linux-arm-msm.vger.kernel.org AND flag:unread AND NOT flag:trashed"
		    :key ?m))
