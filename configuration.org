...
* Setup =vertico=, =consult=, =orderless=, and =marginalia=
  ...
  #+begin_src emacs-lisp
    ;;; The following is mostly taken from `counsel' because I got used to
    ;;; relying on `counsel-git' to finding files in a git repo.
    (defvar fb/git-ls-files-cmd "git ls-files -z --full-name --")

    (defun fb/git-candidates (dir)
    ...
    (defun fb/locate-dominating-file (file &optional dir)
      (and
    ...
    (defun fb/git-find-file ()
      (interactive)
      (let ((default-directory (counsel-locate-git-root)))
	(find-file
	 (completing-read "Find file: "
			  (fb/git-candidates default-directory)))))

    (use-package vertico
      :demand t
      :bind (:map minibuffer-local-map
		  ("<backspace>" . fb/minibuffer-backward-kill))
      :custom
      (vertico-cycle t)
      :init
      (vertico-mode t))
