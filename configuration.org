...
* COMMENT Setup =denote=

   #+begin_src emacs-lisp
     (defun fb/daily-exists-p ()
       "Returns t when we already have a daily for today."
       (let ((files (denote--directory-files))
             (daily-regex (concat
                           (format-time-string "%Y%m%d" (current-time))
                           "T[0-9]\\{6\\}--[a-z0-9-]+__journal.org")))
         (seq-filter (lambda (f) (string-match-p daily-regex f)) files)))

     (defun fb/denote-daily ()
       "Create a new daily."
       (interactive)
       (let ((daily (fb/daily-exists-p)))
         (if daily
             (find-file (car daily))
           (denote
            (denote--title-prompt)
     ...
     (use-package denote
       :custom
       (denote-directory (expand-file-name "~/workspace/org"))
       (denote-known-keywords '("emacs" "programming" "computer science" "programming language" "work"))
